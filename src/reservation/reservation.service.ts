...
export class ReservationService {
  ...
  async createReservation(
    ...
    return await this.dataSource.transaction(async (manager) => {
      ...
      const seat = await manager
        ...
        .where('seat.concertScheduleId = :scheduleId AND seat.seatNumber = :seatNumber', {
      ...
      const now = new Date();
      const expiresAt = new Date(now.getTime() + TEMP_RESERVATION_MINUTES * 60 * 1000);
  ...
  async confirmReservation(
    ...
    if (reservation.expiresAt < new Date()) {
      ...
      throw new BadRequestException('임시 배정이 만료된 예약입니다. 다시 예약해주세요.');
    }
